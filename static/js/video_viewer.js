...
let mediaId = document.getElementById("media_id_value")?.dataset.mediaId;
...
function initializeVideoViewer() {
    mediaId = document.getElementById("media_id_value")?.dataset.mediaId;
    subtitleContainer = document.getElementById("subtitles");
    currentSubtitleDiv = document.getElementById("current-subtitle");
    updateCurrenrSubtitleColumns(4, gap);
    ...
    video.addEventListener("timeupdate", () => {
        ...
        const esSubs = subtitlesData["es"] || [];
    
        if (!subtitleContainer) return;
    
        esSubs.forEach((sub, index) => {
            const div = subtitleContainer.children[index];
            if (!div) return;
    ...
    subtitlesData = { es: [], fr: [], eslong: [], frlong: [] };
    subtitles = [];
    activeTab = "es";
    currentIndex = -1;
    replayingSubtitleIndex = -1;
    displayedSubtitleIndex = -1
    replayingSubtitle = false;
    isEditingEs = false;
    isEditingFr = false;
